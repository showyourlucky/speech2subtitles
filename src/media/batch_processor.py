...
class BatchProcessor:
    ...
    def __init__(
        ...
        logger.info(
            "批量处理器初始化完成: debug_dump_vad_segments=%s, debug_dump_dir=%s",
            self.debug_dump_vad_segments,
            self.debug_dump_dir,
        )
    ...
    def _transcribe_audio(
        ...
        if temp_var:
            ...
            def _build_stream_for_group(group_indices: list[int]) -> tuple[Any, list[dict[str, Any]], float]:
                ...
                时间轴映射用于后续按 token 时间戳对齐字幕，提升时间分配精度。
                """
                chunk_list = []
            ...
            # ============================================================
            # 阶段3: 拆分识别结果 → 生成 ≤MAX_SUBTITLE_DURATION 的字幕段
            # ============================================================
            SENTENCE_PATTERN = re.compile(r'[。！？.!?]')
            CLAUSE_PATTERN = re.compile(r'[，、；：,;:]')

            def _split_text_by_punctuation(text: str, pattern: re.Pattern) -> list[str]:
            ...
            def split_text_to_subtitles(
                ...
                # 第一层: 尝试按句级标点拆分
                parts = _split_text_by_punctuation(text, SENTENCE_PATTERN)

                # 如果句级拆分不够细,继续按子句标点拆分
                if len(parts) < num_parts:
                    expanded = []
                    for p in parts:
                        sub_parts = _split_text_by_punctuation(p, CLAUSE_PATTERN)
                        expanded.extend(sub_parts if sub_parts else [p])
                    parts = expanded

                # 如果仍然不够细,在标点附近智能等分
                if len(parts) < num_parts:
                    fine_parts = _split_fine_by_nearest_break(
                        text, num_parts
                    )
                ...
                if total_chars > 0 and len(parts) > 1:
                    ...
                    aligned_boundaries = _find_audio_aligned_boundaries(
            ...
            def _distribute_text_by_punctuation(
                full_text: str, weights: list[float]
            ) -> list[str]:
                """
                按标点将完整文本分配到多个目标段。
                ...
                # 先按所有标点拆分为最小片段
                all_parts = _split_text_by_punctuation(
                    full_text, SENTENCE_PATTERN
                )
                refined = []
                for p in all_parts:
                    sub = _split_text_by_punctuation(p, CLAUSE_PATTERN)
                ...
                if not all_parts or total_chars == 0 or total_weight <= 0:
                    # 退化为按字符数分配
                    return _distribute_text_by_ratio(
                        full_text, weights
                    )
                ...
                if empty_count > 0:
                    fallback = _distribute_text_by_ratio(full_text, weights)
            ...
            for group_id, (meta, stream) in enumerate(zip(group_info, streams)):
                ...
                if is_long:
                    ...
                    sub_segs = split_text_to_subtitles(
                        text,
                        start_s,
                        duration_s,
                    ...
                    timestamp_weights = _extract_timestamp_weights(result, timeline)
                    ...
                    distributed = _distribute_text_by_punctuation(
                        text,
