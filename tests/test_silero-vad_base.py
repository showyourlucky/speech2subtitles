...
def get_model_path():
    ...
    # 向上一级到项目根目录，然后进入models/silero-vad/
    model_dir = current_dir.parent / "models" / "silero_vad"
    model_path = model_dir / "silero_vad.onnx"
    
    if not model_path.exists():
        # 如果标准模型不存在，尝试量化版本
        model_path = model_dir / "silero_vad.int8.onnx"
        if not model_path.exists():
            raise FileNotFoundError(f"找不到模型文件，请确保以下任一文件存在:\n"
                                  f"  - {model_dir / 'silero_vad_v5.onnx'}\n"
                                  f"  - {model_dir / 'silero_vad.int8.onnx'}")
...
def basic_vad_example():
    """基本VAD检测示例"""
    try:
        print("=== Sherpa-ONNX Silero-VAD 基础测试 ===\n")
        ...
        config.silero_vad.min_silence_duration = 0.1  # 最小静音持续时间（秒）
        config.silero_vad.min_speech_duration = 0.1   # 最小语音持续时间（秒）
        ...
        print(f"  - 检测阈值: {config.silero_vad.threshold}")
        print(f"  - 最小语音持续时间: {config.silero_vad.min_speech_duration}s")
        print(f"  - 最小静音持续时间: {config.silero_vad.min_silence_duration}s")
        ...
        if segment_count == 0:
            ...
            print("  4. 采样率不匹配")
            print("\n建议:")
            print("  - 尝试降低阈值 (如 config.silero_vad.threshold = 0.3)")
        ...
        print(f"文件错误: {e}")
        print("\n请确保:")
        print("  1. 模型文件存在于 models/silero-vad/ 目录下")
        print("  2. 测试音频文件存在")
...
if __name__ == "__main__":
    print("Sherpa-ONNX Silero-VAD 测试程序\n")
